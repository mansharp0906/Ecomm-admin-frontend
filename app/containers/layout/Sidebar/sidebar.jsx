...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  // On mount, read the role
  useEffect(() => {
    const role = localStorage.getItem('userRole'); // assume 'admin' or 'vendor'
    setUserRole(role);
  }, []);
  ...
  // A helper to wrap menu items with a role check
  const renderIfRole = (roles, element) => {
      ...
      >
        ...
        <nav className="pt-6 flex-1 overflow-y-auto no-scrollbar">
          <ul className="list-none">
              ...
              <li>
                ...
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    <ProductIcon />
                    <span
                      className={`transition-opacity ml-3 duration-300 ${
                        sidebarOpen ? 'opacity-100' : 'opacity-0 hidden'
                      } truncate`}
                    >
                      Product Section
            ...
            {/* Promotion Management (optional: for both or one role) */}
            {renderIfRole(
              ['admin', 'vendor'],
              <li>
                <button
                ...
                >
                  ...
                  <span
                    className={`transition-opacity duration-300 mr-1 pl-1 ${
                      sidebarOpen ? 'opacity-100' : 'opacity-0 hidden'
                    } truncate`}
                  >
            ...
            {renderIfRole(
              ...
              <li>
                ...
                >
                  ...
                  <span
                    className={`transition-opacity text-start mr-10 duration-300 ${
                      sidebarOpen ? 'opacity-100' : 'opacity-0 hidden'
                    } truncate`}
                  >
                    Business Settings
            ...
            {renderIfRole(
              ...
              <li>
                <button
                  className="w-full text-left px-4 py-2 flex justify-between items-center hover:bg-gray-200 whitespace-nowrap"
                  onClick={() => toggleDropdown('storeSection')}
                  aria-expanded={openDropdown === 'storeSection'}
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    <StoreIcon />
                    <span
                  ...
                  <MdArrowDropDown
                    className={`inline ml-2 transform transition-transform ${
                      sidebarOpen ? 'opacity-100' : 'opacity-0 hidden'
                    } ${openDropdown === 'storeSection' ? 'rotate-180' : ''}`}
                  />
                </button>

                <ul
                  className={`overflow-hidden transition-all duration-300 ${
                    openDropdown === 'storeSection' ? 'max-h-96' : 'max-h-0'
                  } shadow-xl rounded mt-1 pl-4`}
                >
                  {[
                    { label: 'All Shops', path: '/shops' },
                    // { label: 'Add New Shop', path: '/shops/create' },
                  ].map(({ label, path }) => (
                    <li key={path}>
                      <Link
                        to={path}
